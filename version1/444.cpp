...
int main(int argc, char **argv) {
    ...
    for(int y = 0; y < block_size; y++) {
        // for global variable we have code below
        // int gi = i + block_start; interchange it with y meaning
    ...
    //max_time - max time from all processes
    ...
    if (rank == 0) {
        ...
        for (int src_rank = 0; src_rank < size; src_rank++) {
            ...
            int srcSizeY = mySizeY/size; //same as block_size=array_size/size
            int block_start = src_rank * srcSizeY; //same as block_start=rank*block_size
